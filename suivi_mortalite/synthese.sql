DROP VIEW IF EXISTS gn_monitoring.v_synthese_suivi_mortalite;
CREATE OR REPLACE VIEW gn_monitoring.v_synthese_suivi_mortalite AS WITH source AS (
	SELECT id_source
	FROM gn_synthese.t_sources
	WHERE name_source = CONCAT('MONITORING_', UPPER('suivi_mortalite'))
	LIMIT 1
), sites AS (
	SELECT id_base_site,
...
SELECT *
FROM gn_monitoring.v_synthese_suivi_mortalite